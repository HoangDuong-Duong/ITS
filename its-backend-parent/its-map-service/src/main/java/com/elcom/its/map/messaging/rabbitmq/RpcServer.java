            ...
            if (request != null) {
                ...
                switch (request.getRequestMethod()) {
                    case "GET":
                       ...
                       }else if ("/map/nearby-camera".equalsIgnoreCase(requestPath)) {
                            response = mapController.getListNearbyCamera(request.getRequestPath(), request.getRequestMethod(), urlParam, headerParam); 
                       }else if ("/map/near-by-site".equalsIgnoreCase(requestPath)) {
                           response = mapController.getListCameraNearBySite(request.getRequestPath(), request.getRequestMethod(), urlParam, headerParam);
                       }
                       else if ("/map/straight-map".equalsIgnoreCase(requestPath)) {
                            response = mapController.getStraightMapData(request.getRequestPath(), request.getRequestMethod(), urlParam, headerParam); 
                       }
                       else if ("/map/straight-map/traffic-flow".equalsIgnoreCase(requestPath)) {
                            response = mapController.getStraightMapTrafficData(request.getRequestPath(), request.getRequestMethod(), urlParam, headerParam); 
                       ...
                       }else if ("/map/long-lat".equalsIgnoreCase(requestPath)) {
                            response = mapController.getLongLat(request.getRequestPath(), request.getRequestMethod(), urlParam, headerParam); 
                       }
                        break;
                    case "POST":                         
                        break;
