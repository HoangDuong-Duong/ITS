...
import com.elcom.its.notify.controller.NotifyController;
                    ...
                    case "GET":
                        if ("/notify".equalsIgnoreCase(requestPath)) {
                            if (StringUtils.isNotBlank(pathParam)) {
                                ...
                                response = notifyController.getNotifyByUser(headerParam, requestPath, request.getRequestMethod(), urlParam, bodyParam);
                            }
                        } else if ("/notify/number-notify".equalsIgnoreCase(requestPath)) {
                            response = notifyController.getNumberNotifyByUser(headerParam, requestPath, request.getRequestMethod(), urlParam, bodyParam);
                        }
                        break;
                    case "POST":
                        if ("/notify/send-notify".equalsIgnoreCase(requestPath)) { // Send notify for service to service
                            ...
                            response = notifyController.readOneNotify(headerParam, requestPath, request.getRequestMethod(), urlParam, bodyParam);
                        }
                        break;
                    case "PUT":
                        break;
