package com.elcom.its.id.controller;

import com.elcom.its.id.checkPolicy.ResultCheckDto;
import com.elcom.its.id.config.ApplicationConfig;
import com.elcom.its.id.constant.Constant;
import com.elcom.its.id.mail.MailContentDTO;
import com.elcom.its.id.model.Unit;
import com.elcom.its.id.model.User;
import com.elcom.its.id.model.dto.*;
import com.elcom.its.id.service.TokenService;
import com.elcom.its.id.service.UnitService;
import com.elcom.its.id.service.UserService;
import com.elcom.its.id.thread.IdThreadManager;
import com.elcom.its.id.utils.encrypt.AES;
import com.elcom.its.id.validation.UserValidation;
...
public class UserController extends BaseController {
    ...
    public ResponseMessage getAllUser(Map<String, String> headerParam, String requestPath, String method, String urlParam, Map<String, Object> bodyParam) {
        ...
        if (dto == null) {
            ...
            //Check ABAC quyền xem chi tiet user
            ...
            if (abacResponseDTO != null) {
                if (abacResponseDTO.getStatus()) {
                    ...
                    if (currentPage == 0 || rowsPerPage == 0) {
                        ...
                        if (!StringUtil.isNullOrEmpty(sort) && !"uuid".equalsIgnoreCase(sort) && !"email".equalsIgnoreCase(sort) && !"mobile".equalsIgnoreCase(sort) && !"userName".equalsIgnoreCase(sort) && !"fullName".equalsIgnoreCase(sort) && !"createdAt".equalsIgnoreCase(sort)) {
                            response = new ResponseMessage(HttpStatus.BAD_REQUEST.value(), "Không có kiểu sort theo " + sort, new MessageContent(HttpStatus.BAD_REQUEST.value(), "Không có kiểu sort theo " + sort, null));
                        } else {
                            UserPagingDTO userDTO = userService.findAll(keywordDecode, status, currentPage, rowsPerPage, sort, signupType, mobileVerify, null, startDate, endDate);
                            if (userDTO == null || userDTO.getDataRows() == null || userDTO.getDataRows().isEmpty()) {
                                response = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK.toString(), new MessageContent(HttpStatus.OK.value(), HttpStatus.OK.toString(), null));
                            } else {
                                ...
                                response = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK.toString(), new MessageContent(HttpStatus.OK.value(), HttpStatus.OK.toString(), userList, userDTO.getTotalRows()));
                            }
                        }
                    }
                } else {
                    response = new ResponseMessage(HttpStatus.FORBIDDEN.value(), "Bạn không có quyền quản lý danh sách tài khoản", new MessageContent(HttpStatus.FORBIDDEN.value(), "Bạn không có quyền quản lý danh sách tài khoản", null));
                }
            } else {
    ...
    public ResponseMessage getAllUserInternal() {
        ...
        response = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK.toString(), new MessageContent(HttpStatus.OK.value(), HttpStatus.OK.toString(),userMap));
        return response;
    }
    ...
    public ResponseMessage getDetailUser(String requestUrl, String method, String sId, Map<String, String> headerParam) {
        ...
        if (dto == null) {
            ...
            //Check ABAC quyền xem chi tiet user
            LOGGER.info("Check Abac");
            ResultCheckDto resultCheckDto = authorizeABAC(null, null, dto.getUuid(), requestUrl, "DETAIL");
            LOGGER.info("Nhận kết quả check abac");
            if (resultCheckDto != null) {
                if (resultCheckDto.getStatus()) {
                    ...
                    response = new ResponseMessage(HttpStatus.FORBIDDEN.value(), "Bạn không có quyền xem chi tiết tài khoản", new MessageContent(HttpStatus.FORBIDDEN.value(), "Bạn không có quyền xem chi tiết tài khoản", null));
                }
            } else {
                response = new ResponseMessage(HttpStatus.FORBIDDEN.value(), "Bạn không có quyền xem chi tiết tài khoản", new MessageContent(HttpStatus.FORBIDDEN.value(), "Bạn không có quyền xem chi tiết tài khoản", null));
            }
                    ...
                    if (resultCheckDto != null) {
                        if (resultCheckDto.getStatus()) {
                            ...
                            if ("admin".equalsIgnoreCase(user.getUserName())) {
                                response = new ResponseMessage(HttpStatus.FORBIDDEN.value(), "Bạn không thể xóa user admin của hệ thống", new MessageContent(HttpStatus.FORBIDDEN.value(), "Bạn không thể xóa user admin của hệ thống", null));
                            } else if (dto.getUuid().equals(user.getUuid())) {
                                response = new ResponseMessage(HttpStatus.FORBIDDEN.value(), "Bạn không thể xóa chính mình", new MessageContent(HttpStatus.FORBIDDEN.value(), "Bạn không thể xóa chính mình", null));
                            } else {
                                userService.remove(user);
//                                  User userDelete = userService.deleteByUuid(user.getUuid());
...
//                                response = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK.toString(), new MessageContent(bodyParam));
    ...
    public ResponseMessage updateStatus(Map<String, Object> bodyParam, Map<String, String> headerParam) {
        ...
        if (dto == null) {
            ...
            if (invalidData != null) {
                response = new ResponseMessage(HttpStatus.BAD_REQUEST.value(), invalidData, new MessageContent(HttpStatus.BAD_REQUEST.value(), invalidData, invalidData));
            } else {
                User existUser = userService.findByUuid(uuid);
                if (existUser == null) {
                    response = new ResponseMessage(HttpStatus.NOT_FOUND.value(), "Không tìm thấy tài khoản ứng với uuid " + uuid, new MessageContent(HttpStatus.NOT_FOUND.value(), "Không tìm thấy tài khoản ứng với uuid " + uuid, null));
                } else {
                    Map<String, Object> subject = new HashMap<>();
                    Map<String, Object> attributes = new HashMap<>();
                    ...
                    if (resultCheckDto != null) {
                        if (resultCheckDto.getStatus()) {
                            ...
                            response = new ResponseMessage(HttpStatus.FORBIDDEN.value(), "Bạn không có quyền cập nhật trạng thái tài khoản", new MessageContent(HttpStatus.FORBIDDEN.value(), "Bạn không có quyền cập nhật trạng thái tài khoản", null));
                        }
                    } else {
                        response = new ResponseMessage(HttpStatus.FORBIDDEN.value(), "Module kiểm tra quyền không phản hồi", new MessageContent(HttpStatus.FORBIDDEN.value(), "Module kiểm tra quyền không phản hồi", null));
                    }
        ...
        if (uuidList == null || uuidList.isEmpty()) {
            ...
            try {
                ...
                if (userList == null || userList.isEmpty()) {
                    ...
                    response = new ResponseMessage(HttpStatus.NO_CONTENT.value(), "Không tìm thấy avatar tài khoản ứng với list uuid", new MessageContent(HttpStatus.NO_CONTENT.value(), "Không tìm thấy avatar tài khoản ứng với list uuid", null));
                } else {
                ...
                response = new ResponseMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Lỗi không cập nhật " + ex, new MessageContent(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Lỗi không cập nhật " + ex, ex.toString()));
                ex.printStackTrace();
            }
        }
        return response;
            ...
            if (bodyParam == null || bodyParam.isEmpty()) {
                ...
                if (invalidData != null) {
                    ...
                    if (existUser != null && (StringUtil.isNullOrEmpty(user.getMobile()) || !user.getMobile().equals(mobile))) {
                        response = new ResponseMessage(HttpStatus.CONFLICT.value(), "Đã tồn tại tài khoản trên hệ thống ứng với số điện thoại " + mobile, new MessageContent(HttpStatus.CONFLICT.value(), "Đã tồn tại tài khoản trên hệ thống ứng với số điện thoại " + mobile, null));
                    } else {
                        //Check Redis contains OTP of user
    ...
    public ResponseMessage verifyMobileViaOtp(String requestPath, Map<String, Object> bodyParam, Map<String, String> headerParam) {
        ResponseMessage response = null;
        AuthorizationResponseDTO dto = getAuthorFromToken(headerParam);
        if (dto == null) {
            response = new ResponseMessage(HttpStatus.FORBIDDEN.value(), "Bạn chưa đăng nhập",
            ...
            if (bodyParam == null || bodyParam.isEmpty()) {
                ...
                if (invalidData != null) {
                    ...
                    User user = userService.findByUuid(dto.getUuid());
                    LOGGER.info("Verify Mobile Via OTP - user id: {}, otp: {}, user otp: {}", dto.getUuid(), otp, user.getOtp());
                    if (user != null && otp.equals(user.getOtp())) {
                        ...
                        if (user.getOtpTime() != null && System.currentTimeMillis() <= user.getOtpTime().getTime()) {
                            try {
                                ...
                                response = new ResponseMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Lỗi xác minh số điện thoại qua OTP >>> " + ex, new MessageContent(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Lỗi xác minh số điện thoại qua OTP >>> " + ex, null));
                            }
                        } else {
                            response = new ResponseMessage(HttpStatus.NOT_ACCEPTABLE.value(), "OTP hết hạn", new MessageContent(HttpStatus.NOT_ACCEPTABLE.value(), "OTP hết hạn", null));
                        }
                    } else {
                        response = new ResponseMessage(HttpStatus.NOT_FOUND.value(), "Tài khoản không tồn tại hoặc OTP không chính xác", new MessageContent(HttpStatus.NOT_FOUND.value(), "Tài khoản không tồn tại hoặc OTP không chính xác", null));
                    }
                }
            }
        }
            ...
            if (user != null && user.getMobileVerify() == 0) {
                ...
                if (otpExpired == null || otpExpired.getOtpExpiredTime() < System.currentTimeMillis() || otpExpired.getCountSms() < Constant.MAX_SMS_PER_EXPIRED) {
                    ...
                    if (userService.createOTP(user, otp)) {
                        ...
                        response = new ResponseMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Lỗi gửi lại OTP", new MessageContent(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Lỗi gửi lại OTP", null));
                    }
                } else {
                    String reason = "Chỉ được gửi OTP verify tối đa " + Constant.MAX_SMS_PER_EXPIRED + " lần trong vòng " + (Constant.OTP_TIME_EXIPRED / (60 * 1000) + " phút ");
                    response = new ResponseMessage(HttpStatus.FORBIDDEN.value(), reason, new MessageContent(HttpStatus.FORBIDDEN.value(), reason, null));
                }
            } else if (user != null && user.getMobileVerify() == 1) {
                response = new ResponseMessage(HttpStatus.CONFLICT.value(), "Số điện thoại đã được xác minh rồi", new MessageContent(HttpStatus.CONFLICT.value(), "Số điện thoại đã được xác minh rồi", null));
            } else {
                response = new ResponseMessage(HttpStatus.NOT_FOUND.value(), "Không tìm thấy thông tin tài khoản", new MessageContent(HttpStatus.NOT_FOUND.value(), "Không tìm thấy thông tin tài khoản", null));
            }
        }
        return response;
    }
        ...
        if (bodyParam == null || bodyParam.isEmpty()) {
            ...
            if (StringUtil.isNullOrEmpty(token)) {
                ...
                if (StringUtil.isNullOrEmpty(decryptResult)) {
                    response = new ResponseMessage(HttpStatus.BAD_REQUEST.value(), "Token không đúng", new MessageContent(HttpStatus.BAD_REQUEST.value(), "Token không đúng", null));
                } else {
                    ...
                    if (decryptInfo == null || decryptInfo.length < 2) {
                        ...
                        if (user == null || (user.getIsDelete() != null && (user.getStatus() == -1 || user.getIsDelete() == 1))) {
                            ...
                            if (expiredTime < now) {
                                response = new ResponseMessage(HttpStatus.REQUEST_TIMEOUT.value(), "Link quên mật khẩu hết hạn", new MessageContent(HttpStatus.REQUEST_TIMEOUT.value(), "Link quên mật khẩu hết hạn", null));
                            } else {
    ...
    public ResponseMessage checkOtpPassword(Map<String, Object> bodyParam, Map<String, String> headerParam) {
        ResponseMessage response = null;
        if (bodyParam == null || bodyParam.isEmpty()) {
            response = new ResponseMessage(HttpStatus.BAD_REQUEST.value(), Constant.VALIDATION_INVALID_PARAM_VALUE, new MessageContent(HttpStatus.BAD_REQUEST.value(), Constant.VALIDATION_INVALID_PARAM_VALUE, null));
        } else {
            String email = (String) bodyParam.get("email");
            String mobile = (String) bodyParam.get("mobile");
            String otpPassword = (String) bodyParam.get("otp");
            if (StringUtil.isNullOrEmpty(otpPassword) || (!StringUtil.validateEmail(email) && !StringUtil.checkMobilePhoneNumberNew(mobile))) {
                response = new ResponseMessage(HttpStatus.BAD_REQUEST.value(), Constant.VALIDATION_INVALID_PARAM_VALUE, new MessageContent(HttpStatus.BAD_REQUEST.value(), Constant.VALIDATION_INVALID_PARAM_VALUE, null));
            } else {
                String userInfo = StringUtil.checkMobilePhoneNumberNew(mobile) ? mobile : email;
                User user = userService.findByEmailOrMobileAndOTPPassword(userInfo, otpPassword);

                if (user == null || (user.getIsDelete() != null && (user.getStatus() == -1 || user.getIsDelete() == 1))) {
                    response = new ResponseMessage(HttpStatus.NOT_FOUND.value(), "Không tìm thấy thông tin tài khoản hoặc OTP không đúng", new MessageContent(HttpStatus.NOT_FOUND.value(), "Không tìm thấy thông tin tài khoản hoặc OTP không đúng", null));
                } else {
                    if (user.getOtpPasswordTime() != null && System.currentTimeMillis() <= user.getOtpPasswordTime().getTime()) {
                        response = new ResponseMessage(new MessageContent(bodyParam));
                    } else {
                        response = new ResponseMessage(HttpStatus.REQUEST_TIMEOUT.value(), "OTP quên mật khẩu hết hạn", new MessageContent(HttpStatus.REQUEST_TIMEOUT.value(), "OTP quên mật khẩu hết hạn", null));
                    }
                }
            }
        }
    ...
    //Kieu OTP
    public ResponseMessage updateForgotPasswordForApp(Map<String, Object> bodyParam, Map<String, String> headerParam) {
        ResponseMessage response = null;
        if (bodyParam == null || bodyParam.isEmpty()) {
            response = new ResponseMessage(HttpStatus.BAD_REQUEST.value(), Constant.VALIDATION_INVALID_PARAM_VALUE, new MessageContent(HttpStatus.BAD_REQUEST.value(), Constant.VALIDATION_INVALID_PARAM_VALUE, null));
        } else {
            ...
            String invalidData = new UserValidation().validateUpdateForgotPasswordForApp(otpPassword, email, mobile, newPassword, rePassword);
            if (invalidData != null) {
                response = new ResponseMessage(HttpStatus.BAD_REQUEST.value(), invalidData, new MessageContent(HttpStatus.BAD_REQUEST.value(), invalidData, null));
            } else {
                String userInfo = StringUtil.checkMobilePhoneNumberNew(mobile) ? mobile : email;
                ...
                if (user == null || (user.getIsDelete() != null && (user.getStatus() == -1 || user.getIsDelete() == 1))) {
                    ...
                    if (user.getOtpPasswordTime() != null && System.currentTimeMillis() <= user.getOtpPasswordTime().getTime() + 60 * 1000) {
                        //Update password
                        user.setPassword(new BCryptPasswordEncoder().encode(newPassword));
                        user.setSetPassword(1);
                        try {
                            if (userService.changePassword(user)) {
                                ...
                                Authentication authentication = null;
                                try {
                                    authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(userInfo, newPassword));
                                } catch (AuthenticationException ex) {
                                    LOGGER.error("Error to set new authentication >>> " + ex);
                                }
                                // Set thông tin authentication mới vào Security Context
    ...
    public ResponseMessage tailOtpLog() {
        String last20Line = getLastNLogLines(new File("/data/otp/log.log"), 20);
        ResponseMessage response = new ResponseMessage(new MessageContent(last20Line.split("\n")));
        return response;
    }
