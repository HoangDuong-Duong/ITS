...
public class UserCustomizeRepository extends BaseRepository {
    ...
    public User findByEmail(String email) {
        ...
        Object result = null;
        try {
            Query query = session.createNativeQuery(" SELECT * FROM user WHERE email = ? AND is_delete = 0", User.class);
    ...
    public User findByMobile(String mobile) {
        ...
        Object result = null;
        try {
            Query query = session.createNativeQuery("SELECT * FROM user WHERE mobile = ? AND is_delete = 0", User.class);
    ...
    public User findByUserName(String userName) {
        ...
        Object result = null;
        try {
            Query query = session.createNativeQuery("SELECT * FROM user WHERE user_name = ? AND is_delete = 0", User.class);
            query.setParameter(1, userName);
            result = query.getSingleResult();
        } catch (Exception ex) {
            LOGGER.error(ex.toString());
    ...
    public User findByEmailOrMobileOrUserName(String userInfo) {
        ...
        try {
            Query query = session.createNativeQuery("SELECT * FROM user WHERE "
                    ...
                    + " OR (user_name = ? AND is_delete = 0) ", User.class);
            query.setParameter(1, userInfo);
            query.setParameter(2, userInfo);
    ...
    public User findBySocial(Integer signupType, String socialId) {
        Session session = openSession();
        try {
            String sql = "FROM User u WHERE u.signupType = :signupType AND u.isDelete = 0 ";
            switch (signupType) {
    ...
    public User findByEmailOrMobileAndOTPPassword(String userInfo, String otpPassword) {
        Session session = openSession();
        Object result = null;
        try {
            Query query = session.createNativeQuery("SELECT * FROM user WHERE "
                    + " ((email = ? AND is_delete = 0) OR (mobile = ? AND is_delete = 0)) "
                    + " AND otp_password = ? ", User.class);
            query.setParameter(1, userInfo);
            query.setParameter(2, userInfo);
            query.setParameter(3, otpPassword);
            result = query.getSingleResult();
        } catch (Exception ex) {
            LOGGER.error(ex.toString());
        } finally {
    ...
    public List<String> findAllByGroupCameraIdList(List<String> groupCameraIdList) {
        Session session = openSession();
        try {
            String sql = " SELECT u.uuid as uuid FROM user u JOIN groups_camera_group gcg "
                    + " ON u.groups_uuid = gcg.groups_uuid WHERE gcg.groups_camera_uuid IN :groupCameraIdList";
            return (List<String>) session.createNativeQuery(sql)
                    .setParameter("groupCameraIdList", groupCameraIdList)
