...
public interface JobRepository extends PagingAndSortingRepository<Job, String> {
    ...
    @Query("select j from Job j WHERE (j.startTime between :startTime and :endTime) or (j.modifiedTime between :startTime and :endTime)")
    List<Job> getJobReport(@Param("startTime") Date startTime, @Param("endTime") Date endTime);
    ...
    Page<Job> findByJobTypeInAndStartTimeBetween(List<String> jobType, Date startTime, Date endTime, Pageable pageable);
}
