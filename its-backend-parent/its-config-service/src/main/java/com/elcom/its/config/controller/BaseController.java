...
import com.elcom.its.config.messaging.rabbitmq.RabbitMQClient;
import com.elcom.its.config.messaging.rabbitmq.RabbitMQProperties;
...
public class BaseController {
        ...
        if (result != null) {
            ...
            if (response != null && response.getStatus() == HttpStatus.OK.value()) {
                try {
                    ...
                    LOGGER.info("Lỗi giải mã AuthorizationResponseDTO khi gọi user service verify: " + ex);
                    return null;
                }
            } else {
                //Forbidden
    ...
    public void publishDeletedListStage(List<String> listStageIds) throws JsonProcessingException {
        DeleteDataPublishMessage message = DeleteDataPublishMessage.builder()
                .dataType("STAGE")
                .dataId(String.join(",", listStageIds))
                .data(listStageIds)
                .build();
    ...
    public void publishDeletedStage(String stageId) throws JsonProcessingException {
        DeleteDataPublishMessage message = DeleteDataPublishMessage.builder()
                .dataType("STAGE")
                .dataId(stageId)
                .data(new ArrayList<>(List.of(stageId)))
                .build();
        rabbitMQClient.callPublishService(RabbitMQProperties.DELETE_DATA_PUBLISH_EXCHANGE, RabbitMQProperties.DELETE_DATA_PUBLISH_KEY, message.toJsonString());
    }
}
